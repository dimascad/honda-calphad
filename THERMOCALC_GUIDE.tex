...
\bottomrule
\end{tabular}
\caption*{\footnotesize $^*$At 1273--1873K, SiO$_2$ is in the CRISTOBALITE or TRIDYMITE region (QUARTZ only stable below 846K); TC handles phase selection automatically. $^{**}$FeO uses HALITE structure (NaCl-type) in SSUB.}
\end{table}

\subsubsection{Stoichiometry Reference}

The mole fractions come from the oxide formula (atoms of each element / total atoms):
\begin{align*}
...
\text{CuO, MgO, FeO}: \quad & \frac{1}{1+1} = 0.5 \text{ metal}, \quad \frac{1}{1+1} = 0.5 \text{ O} \\
\text{SiO}_2\text{, TiO}_2: \quad & \frac{1}{1+2} = 0.3333 \text{ metal}, \quad \frac{2}{1+2} = 0.6667 \text{ O}
\end{align*}
...
\textbf{Console Mode (alternative, example for Cu$_2$O):}
\begin{lstlisting}
go poly
s-sys Cu O
s-c t=1273 p=101325 n=1 x(cu)=0.6667 x(o)=0.3333
c-e
lis,,,,
\end{lstlisting}
...
\item Axis: vary X(Cu) from 0 to 0.3, with X(Al) = 0.4 $-$ X(Cu)
\item This walks from pure Al$_2$O$_3$ toward Cu-rich compositions while maintaining 60\% oxygen
\end{enumerate}
...
# Process each oxide (filename, O2 factor in reaction)
process_oxide('cu2o_dGf_1273-1873K.txt', 0.5)   # 2Cu + 1/2 O2 -> Cu2O
process_oxide('al2o3_dGf_1273-1873K.txt', 1.5)  # 4/3 Al + O2 -> 2/3 Al2O3
process_oxide('mgo_dGf_1273-1873K.txt', 0.5)    # Mg + 1/2 O2 -> MgO
process_oxide('sio2_dGf_1273-1873K.txt', 1.0)   # Si + O2 -> SiO2
process_oxide('tio2_dGf_1273-1873K.txt', 1.0)   # Ti + O2 -> TiO2
process_oxide('feo_dGf_1273-1873K.txt', 0.5)    # Fe + 1/2 O2 -> FeO
\end{lstlisting}
