...
\begin{itemize}[leftmargin=*]
    ...
    \item Dr.\ Zhang's suggestion of adding Na$_2$S flux is exactly this: modifying the slag ``ionic atmosphere'' to improve Cu capture
\end{itemize}
...
Phases 4--6 were skipped because Phase 3 failed. The method dump of \texttt{DiffusionIsoThermalCalculation} (20 methods) was captured but does not include \texttt{Region} methods. The next run must probe \texttt{Region} methods via \texttt{dir()} to identify the correct phase-assignment API (likely \texttt{with\_phase()}).

\subsubsection{Implications}

The DICTRA API is confirmed available with 40 working database pairs. The remaining blocker is a single missing API call: telling the \texttt{Region} which phase to use as the diffusion matrix. This is a usage issue, not a fundamental limitation. Once the correct method is identified (via \texttt{dir(Region(...))}), single-region Cu diffusion calculations should work, enabling the steel/slag interface model.
...
\subsection{March 14, 2026: Unified Workbook Build}

Built the master deliverable: \texttt{Cu\_Removal\_Unified.xlsx} (15 tabs, 3-tier color coding).

\textbf{Structure:}
\begin{itemize}[leftmargin=*]
    \item \textbf{Blue (Analysis):} Dashboard, Screening\_Summary, CuFe2O4\_DeepDive, Activity\_and\_Slag, Normalization --- formula-driven tabs with traceable cell references
    \item \textbf{Green (Results):} Screening\_Table, Ternary\_Verdicts, Activity\_Corrected --- derived CSVs
    \item \textbf{Gray (Raw):} 7 tabs of raw TC-Python CSV output (the ``receipt'')
\end{itemize}

\textbf{Script:} \texttt{screening/build\_unified\_xlsx.py} ($\sim$1600 lines). Uses openpyxl with IEEE formatting (dark header, alternating gray rows, thick border hierarchy).

\textbf{Key design principle:} Every number in a blue analysis tab traces back to a gray raw tab via Excel formulas (e.g., \texttt{=Ternary\_Rxns!N42}). Click any cell to see where the data came from.

\textbf{Bugs found and fixed:}
\begin{enumerate}[leftmargin=*]
    \item Cell D7 in Activity\_and\_Slag started with ``='' causing \texttt{\#NAME?} error (Excel interpreted Unicode $\times$ and $\gamma$ as functions)
    \item $\Delta G_{\text{pure}}$ values hardcoded as floats instead of cross-tab formula references
    \item Sensitivity sweep baked literal numbers into formula strings instead of referencing Section~2 cells
    \item Activity\_Corrected tab had binary verdict logic; upgraded to 4-tier (ROBUST/MARGINAL/UNCERTAIN/UNFAVORABLE) via \texttt{IF} formula on column H
\end{enumerate}
...
\hline
\rowcolor{altrow}
\texttt{build\_unified\_xlsx.py} & Master 15-tab workbook: 5 analysis + 3 results + 7 raw data tabs with traceable formulas. \\
\hline
